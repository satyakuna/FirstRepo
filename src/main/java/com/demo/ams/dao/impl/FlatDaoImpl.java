	...
	public void addFlatDetails(final List<FlatDO> flatList) {
		 ...
		 public JdbcTemplate getJtemplate() {
			...
			public String linkResident(FlatDO flatdo) {
				String SQL="update APT_DETAIL set owner_id="+flatdo.getResidentID()+" where " +
						"flatno='"+flatdo.getFlatNo()+"'";
			...
			public String linkBulkResident(List residentList){
				...
				int x[]=jtemplate.batchUpdate("update APT_DETAIL set owner_id=? where flatno=? ", new BatchPreparedStatementSetter() {
			...
			public String linkBulkTenant(List tenantList){
				String SQL="";
				System.out.println("Inside PTA");		
				 SQL = "select a.TENANT_NAME ,a.TE_ADD_COMM,a.TE_CONTACT_NO,a.TE_ALT_CONTACT_NO,a.TE_EMAIL_ID,a.ID_PROOF_NO,a.ID_PROOF_TYPE,"
				 		+ "a.OWNER_ID,a.FLAT_ID from "+
		               " TENANT_REQUEST a where a.REQID=? ";
				 ptado = j = jtemplate.query(SQL,new Object[]{flatID},new WorkerMapper());      
			      System.out.println("size of residentdo ::::"+ptado.size());
				
				int x[]=jtemplate.batchUpdate("update APT_DETAIL set tenant_id=? where flatno=? ", new BatchPreparedStatementSetter() {

					   public void setValues(PreparedStatement ps, int i)
					     throws SQLException {
						   FlatDO flatDO  =(FlatDO)tenantList.get(i);
					    ps.setInt(1, Integer.parseInt(flatDO.getResidentID()));
					    ps.setString(2, flatDO.getFlatNo());;
				...
				int y[]=jtemplate.batchUpdate("update TENANT_DETAIL set isverified='Y',isactive='Y' where tenant_id=? ", new BatchPreparedStatementSetter() {
